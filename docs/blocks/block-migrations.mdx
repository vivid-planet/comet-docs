...
The structure of a block may change multiple times during its lifetime.
...
A migration is described by a class that extends the `BlockMigration` class and implements the `BlockMigrationInterface`.
...
The file's name should start with the migration verson, for instance `1-change-eyebrow.migration.ts`.
...
-   **adding** a _required_ field to the block
...
-   **adding** an additionally supported block to a blocks block or an one of block
-   **removing** a supported block from a blocks block or an one of block
