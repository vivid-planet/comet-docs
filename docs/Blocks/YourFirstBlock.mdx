...
Blocks are the primary building patterns of a Comet CMS application. In this guide, you will learn how to create your first block, from beginning to end.
...
![HeadlineBlockAdminSite](@site/static/img/HeadlineBlockAdminSite.png)
*The Headline block we’re going to create. On the left side, you can see a preview of the block in the frontend (site). On the right side, you can see the admin component used to fill the block with content.*
...
Note how we use the `RichTextBlock`, a Comet block, to create our Headline block. Composing a block out of child blocks and additional fields is no problem!
...
![HeadlineBlockAdminSite](@site/static/img/HeadlineBlockAdmin.png)
*The admin component of the Headline block*
...
Note how we are using `createBlocksBlock` once again? The helper is available for both the API and the admin part of your Comet application!
...
![HeadlineBlockSite](@site/static/img/HeadlineBlockSite.png)
*The Headline block rendered in the site*
...
In this guide, we’ve learned how to create a block in a Comet CMS project from start to end. We first defined the structure of the block’s data in the API, then created the admin component for users to interact with the block, and finally added the block to the site (frontend).

Now you should be ready to create additional blocks! Maybe even a Dog Ear block?

:::info

We’ve only covered the basic concepts of Comet blocks in this guide. To get a full picture, please review the [blocks](../Blocks/) section in our documentation.
